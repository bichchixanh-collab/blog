...
export function linkify(s) {
  return esc(s).replace(/\*\*([^*<>]+)\*\*/g, '<b>$1</b>').replace(/(https?:\/\/[^\s<>"']+)/g, '<a href="$1" target="_blank" rel="noopener">$1</a>');
}
export function hotSpan() {
  return '<span style="background:#0066cc;color:#fff;font-size:8px;padding:2px 5px;border-radius:8px">HOT</span>';
}
...
export function detailBody(g) {
  ...
  return `<h3>📝 Giới thiệu</h3><p>${linkify(g.desc || '')}</p><h3>🖼️ Hình ảnh</h3>${shotsHtml}`
    + `<h3>⚙️ Yêu cầu</h3><p style="font-size:13px">• <b>MIDP 2.0</b> • Màn hình <b>${esc((g.res || []).join(', '))}</b> • Trống ≥<b>${esc(g.size)}</b> • Opera Mini</p>`;
}
...
export function relatedHtml(games, g, gameHrefBase = '') {
  return related(games, g).map((x) =>
    `<a href="${esc(gameHrefBase)}/game/${esc(x.id)}.html" style="background:#fff;border:1.5px solid #b8d8f8;border-radius:10px;padding:6px;text-align:center;text-decoration:none">`
    ...
    + `<span style="font-size:10px;font-weight:700;display:block;color:#1a3a5c">${esc(String(x.name || '').split('[')[0].slice(0, 14))}</span>`
    + `<small style="font-size:9px;color:#4a7a9a">${esc(((x.res || [])[0] || ''))}</small></a>`
  ).join('');
}
