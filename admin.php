    ...
    if($upThumb===false){ $msg=$upErr; $msgType='error'; }
        ...
        if($upErr){ $msg=$upErr; $msgType='error'; }
            ...
            if ($gateType === 'stats') {
                ...
                $rq = [];
                if (isset($_POST['rq_minutes_on'])) $rq['minutes'] = max(1, min(100000, (int)($_POST['rq_minutes'] ?? 30)));
...
<div class="wrap tab-pane active" id="tab-game">
  ...
  <div class="card" style="height:fit-content">
    ...
    <form method="post" id="gameForm" enctype="multipart/form-data">
      ...
      <script>function insertCredit(){var ta=document.querySelector('textarea[name=desc]');if(!ta)return;if(ta.value.indexOf('[credit]')>=0){alert('Mô tả đã có [credit] rồi.');try{ta.focus();}catch(e){}return;}var tpl='[credit]Game java hay - vào ngay https://J2ME.VERCEL.APP\nVui lòng ghi thêm 2 dòng này khi chia sê lại bài viết.[/credit]\n';try{var s=ta.selectionStart||ta.value.length,e=ta.selectionEnd||ta.value.length;ta.value=ta.value.slice(0,s)+tpl+ta.value.slice(e);}catch(err){ta.value+=tpl;}try{ta.focus();}catch(e){}}</script>
      ...
      <div style="background:#fffbe6;border:1px solid #e6c87a;border-radius:6px;padding:8px;display:grid;gap:6px">
        ...
        <div id="gateStats" style="display:<?=($egT==='stats'?'grid':'none')?>;gap:6px">
          <div style="display:flex;gap:6px;align-items:center"><label style="flex:1;font-size:11px"><input type="checkbox" name="rq_minutes_on" <?=isset($egR['minutes'])?'checked':''?>> ⏱ Phút online</label><input type="number" name="rq_minutes" min="1" max="100000" value="<?=htmlspecialchars($egR['minutes']??30)?>" style="width:90px"></div>
