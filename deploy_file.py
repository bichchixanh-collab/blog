...
def push_selected_files(remote_url):
    """
    Push chỉ những file được chỉ định (nhập tên, cách nhau bằng dấu cách)
    thay vì push toàn bộ thư mục. Vẫn commit + force-push lên nhánh main
    như lựa chọn 2, nhưng chỉ 'git add' đúng các file được liệt kê.
    """
    print(f"\n📦 Chuẩn bị push CÁC FILE CHỈ ĐỊNH trong:\n    {REPO_ROOT}\n  lên: {remote_url}")
    raw = input("Nhập tên các file muốn push (cách nhau bằng dấu cách, vd: file1.html file2.js): ").strip()
    if not raw:
        print("❌ Chưa nhập file nào. Đã huỷ.")
        return

    filenames = raw.split()

    # Kiểm tra file có tồn tại trong REPO_ROOT không, báo lỗi rõ nếu thiếu
    missing = []
    existing = []
    ...
    if missing:
        print("\n⚠️  Không tìm thấy các file sau trong thư mục hiện tại:")
        for fname in missing:
            print(f"   - {fname}")

    if not existing:
        print("❌ Không có file hợp lệ nào để push. Đã huỷ.")
        return

    print("\n✅ Sẽ push các file sau:")
    for fname in existing:
        print(f"   - {fname}")

    if not confirm("Xác nhận push các file trên (sẽ FORCE PUSH, ghi đè nhánh main trên remote)?"):
        print("Đã huỷ.")
        return
    ...
    # Chỉ add đúng các file được chỉ định (mỗi file bọc trong dấu ngoặc kép
    # để xử lý tên file có khoảng trắng/ký tự đặc biệt)
    quoted_files = " ".join(f'"{f}"' for f in existing)
    run(f"git add -- {quoted_files}")

    diff_check = run("git diff --cached --quiet", check=False)
    if diff_check.returncode != 0:
        commit_msg = input("Nhập nội dung commit (Enter để dùng mặc định): ").strip()
        if not commit_msg:
            commit_msg = "Update selected files"
        run(f'git commit -m "{commit_msg}"')
    else:
        print("ℹ️  Không có thay đổi mới trong các file đã chọn để commit, sẽ vẫn thử push nhánh hiện có.")
    ...
    if push_result.returncode != 0:
        ...
        if "GH013" in combined_output or "push protection" in combined_output.lower():
            ...
            print(f"   3. Xoá thư mục .git trong {REPO_ROOT} để xoá sạch lịch sử")
            print("      cũ đang chứa token (rmdir /s /q .git trên Windows), rồi")
            print("      chạy lại script này để push lại từ đầu.")
        else:
            print("\n❌ Push thất bại. Một số nguyên nhân thường gặp:")
            print("   • Chưa đăng nhập / sai quyền truy cập repo GitHub — thử đăng")
            print("     nhập lại bằng: git credential-manager github login")
            ...
            print("   • Mất kết nối mạng tới github.com.")
        sys.exit("Xem chi tiết lỗi Git ở phía trên để biết chính xác nguyên nhân.")
    print("✅ Đã push các file đã chọn lên GitHub thành công.")


def push_specific_folder(remote_url):
    """
    Push chỉ 1 thư mục con cụ thể (nhập tên folder) lên GitHub.
    Sử dụng git subtree hoặc filter-branch logic: tạo commit chỉ chứa
    nội dung folder đó rồi force-push lên main.
    """
    print(f"\n📦 Chuẩn bị push 1 THƯ MỤC CỤ THỂ từ:\n    {REPO_ROOT}\n  lên: {remote_url}")
    folder_name = input("Nhập tên thư mục con muốn push (vd: dist, public, build): ").strip()
    if not folder_name:
        print("❌ Chưa nhập tên thư mục. Đã huỷ.")
        return

    folder_path = os.path.join(REPO_ROOT, folder_name)
    if not os.path.exists(folder_path) or not os.path.isdir(folder_path):
        print(f"❌ Thư mục '{folder_name}' không tồn tại trong {REPO_ROOT}")
        return

    print(f"\n✅ Sẽ push toàn bộ nội dung thư mục: {folder_name}/")
    if not confirm("Xác nhận push thư mục này (sẽ FORCE PUSH, ghi đè nhánh main trên remote)?"):
        print("Đã huỷ.")
        return
    ...
    # Sử dụng git subtree split để tách folder thành commit riêng,
    # sau đó push commit đó lên main
    print(f"\n🔄 Đang tách thư mục '{folder_name}' thành commit riêng...")
    split_result = run(f"git subtree split --prefix={folder_name} -b deploy-{folder_name}", check=False)
    if split_result.returncode != 0:
        print("❌ Không thể tách thư mục (có thể thư mục rỗng hoặc chưa được track bởi git).")
        print("   Thử 'git add -A' trước rồi chạy lại, hoặc kiểm tra thư mục có file không.")
        return

    push_result = run(f"git push origin deploy-{folder_name}:main --force", check=False)
    if push_result.returncode != 0:
        combined_output = (push_result.stdout or "") + (push_result.stderr or "")
        if "GH013" in combined_output or "push protection" in combined_output.lower():
            print("\n🔒 GitHub CHẶN push vì phát hiện secret (token/API key) nằm")
            print("   trong lịch sử commit đang được push.")
            print("   Cách xử lý: thu hồi token, xoá khỏi file, thêm vào .gitignore,")
            print(f"   xoá .git trong {REPO_ROOT}, rồi chạy lại script.")
        else:
            print("\n❌ Push thất bại. Xem chi tiết lỗi Git ở trên.")
        # Dọn nhánh tạm
        run(f"git branch -D deploy-{folder_name}", check=False)
        sys.exit("Xem chi tiết lỗi Git ở phía trên.")
    
    # Dọn nhánh tạm sau khi push thành công
    run(f"git branch -D deploy-{folder_name}", check=False)
    print(f"✅ Đã push thư mục '{folder_name}' lên GitHub thành công.")
...
def main():
    ...
    print("  3 = Push chỉ file chỉ định (nhập tên file, vd: file1 file2 file3)")
    print("  4 = Push 1 thư mục cụ thể (nhập tên folder, vd: dist, public, build)")
    ...
    if choice == "1":
        ...
        push_selected_files(remote_url)
    elif choice == "4":
        push_specific_folder(remote_url)
    else:
        sys.exit("❌ Lựa chọn không hợp lệ. Chỉ nhập 1, 2, 3 hoặc 4.")
