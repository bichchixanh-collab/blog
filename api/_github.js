...
async function readFile({ file }) {
  ...
  if (r.code === 404) return { json: {}, sha: null };
  if (r.code !== 200) throw new Error('github read ' + r.code);
  const j = JSON.parse(r.body);
  return { json: JSON.parse(Buffer.from(j.content, 'base64').toString('utf-8')), sha: j.sha };
}
...
module.exports = { writeFile, readFile, ALLOWED_FILES: [...ALLOW] };
